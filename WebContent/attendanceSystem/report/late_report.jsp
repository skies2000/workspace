...
<script>
function late_report_input_submit(f){
	if(confirm('등록하시겠습니까?')){
		var url = 'late_report_input.hwan';
		f.action = url;
		f.target='main';
		f.submit();
		self.close();
...
<h2>사유서</h2><div></div>
	<form name=frm method=post id = late_report_frm>
		...
		<label class=lLabelSize>과정명</label><input type=text name=studyName><br/>
		...
		<label class=lLabelSize>내용</label><textarea rows="10" cols="30" name=content ></textarea><br/>
		<div id=footarea>
		...
		<input type=button value='등록' onclick="late_report_input_submit(this.form)">
